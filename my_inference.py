...
def main():
    ...
    model = my_nn.DiffusionWriter(num_layers=args.num_attlayers, c1=C1, c2=C2, c3=C3)
    ...
    org_img = my_utils.pad_img(org_img, 2100, 96)
    writer_img = tf.expand_dims(org_img, 0)

    print('writer_img', writer_img.shape)
    style_vector = style_extractor(writer_img)
    a = my_utils.run_batch_inference(model, beta_set, args.textstring, style_vector, 
