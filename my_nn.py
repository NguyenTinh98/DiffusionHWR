...
def loss_fn(eps, score_pred, pl, pl_pred, abar, bce, penalty = 1):
    ...
    return penalty * score_loss + pl_loss, score_loss, pl_loss
...
class StyleExtractor(Model):
    ...
    def __init__(self, height = 96, width = 2100, channel = 3):
        super().__init__()
        self.mobilenet = MobileNetV2(include_top=False, pooling=None, weights='imagenet', input_shape=(height, width, channel))
...
class Text_Style_Encoder(Model):
    def __init__(self, d_model, d_emb = 169, d_ff=512):
        super().__init__()
        # self.emb = Embedding(73, d_model)
        self.emb = Embedding(d_emb, d_model)
...
class DiffusionWriter(Model):
    def __init__(self, num_layers=4, c1=128, c2=192, c3=256, d_emb = 169, drop_rate=0.1, num_heads=8):
        super().__init__()
        ...
        self.text_style_encoder = Text_Style_Encoder(c2*2, d_emb, c2*4)
